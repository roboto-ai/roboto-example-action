...
image_push_args+=(check_magnetometer_norm:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push check_magnetometer_norm:latest to Roboto's private registry"
    exit 1
fi

echo "Creating/updating check_magnetometer_norm action"
create_args=(
