...
echo "Pushing create_derivative_topic:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(create_derivative_topic:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push create_derivative_topic:latest to Roboto's private registry"
    exit 1
fi

echo "Creating/updating create_derivative_topic action"
create_args=(
