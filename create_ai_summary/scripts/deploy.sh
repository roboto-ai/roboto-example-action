...
echo "Pushing create_ai_summary:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(create_ai_summary:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push create_ai_summary:latest to Roboto's private registry"
    exit 1
fi

echo "Creating/updating create_ai_summary action"
create_args=(
