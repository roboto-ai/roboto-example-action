...
if [ "$QUIET" = true ]; then
    docker "${build_subcommand[@]}" --quiet -f $PACKAGE_ROOT/Dockerfile -t create_ai_summary:latest $PACKAGE_ROOT &> /dev/null
else
    docker "${build_subcommand[@]}" -f $PACKAGE_ROOT/Dockerfile -t create_ai_summary:latest $PACKAGE_ROOT
fi
