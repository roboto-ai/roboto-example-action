...
if [ "$QUIET" = true ]; then
    docker "${build_subcommand[@]}" --quiet -f $PACKAGE_ROOT/Dockerfile -t example_email_action:latest $PACKAGE_ROOT &> /dev/null
else
    docker "${build_subcommand[@]}" -f $PACKAGE_ROOT/Dockerfile -t example_email_action:latest $PACKAGE_ROOT
fi
