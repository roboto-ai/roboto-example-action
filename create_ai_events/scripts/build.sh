...
if [ "$QUIET" = true ]; then
    docker "${build_subcommand[@]}" --quiet -f $PACKAGE_ROOT/Dockerfile -t create_ai_events:latest $PACKAGE_ROOT &> /dev/null
else
    docker "${build_subcommand[@]}" -f $PACKAGE_ROOT/Dockerfile -t create_ai_events:latest $PACKAGE_ROOT
fi
