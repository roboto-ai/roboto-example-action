...
Successfully installed uv-0.9.5
  ...
  ╰─▶ Because only library<=1.2.3 is available and create_ai_events depends on library>=2.3.4, we can conclude that your requirements are unsatisfiable.
```
...
- **Use modules for complex logic**: Break complex actions into separate modules under `src/create_ai_events/`
